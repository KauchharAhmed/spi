  ...
  #------------------------------ SECURITY DOR-------------------------#
   ...
   if($door_enter_check == '1'){
    ...
    if($door_studentIdCount > 0){
     ...
     $door_data_student['device_number']  = $device_id;
...
$data['on_created_at'] = $crated_date_is;
...
$data=array();
$data['out_time']     = $current_time ;
$take_attendent       = DB::table('teacher_attendent')->where('teacherId',$teacher_id)
->where('type',$teacher_type)->where('year',$year)->where('shift_id',$shift)->where('dept_id',$dept_id)->where('semister_id',$semister)->where('section_id',$section)->where('day',$day)->where('class_no',$class_id)->where('status',2)->where('created_at',$rcdate)->update($data);
exit();
 }
else{
        echo "Reapet--------------";
