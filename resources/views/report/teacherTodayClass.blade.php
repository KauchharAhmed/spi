...
<div>
 <span style="background-color: #943924f7;text-align: center;color: white;font-weight: bold;font-family: monospace; padding: 15px; margin-left: 29px;">TODAY YOUR CLASS (<?php $in_shift = DB::table('shift')->where('id',$shift_id)->first();
...
<div class="m-portlet ">
    <div class="m-portlet__body  m-portlet__body--no-padding">
     ...
     <?php foreach ($result as  $value) { ?>
        ...
        ->where('routine.semister_id', $semisterr->id)
         ...
         ->where('routine.teacher_id', $session_teacher_id)
    ...
    ->where('teacher_id', $session_teacher_id)
    ->limit(1)
    ->count();
      ...
      $class_id = $get_classs->id ;
       ...
       <?php foreach ($routine as $routinee) { ?>
         ...
         ">
           ...
           <a target="_blank" href="{{URL::to('viewStudentAttendentListByTeacher/'.$routinee->id.'/'.$rcdate)}}">View</a>
           <?php } ?>
            <br/>
            <?php 
             echo $routinee->name ;
